...
var hashCondition = function(condition) {
    var hash = [];
    switch (typeof condition) {
        case 'object':
            if (condition instanceof Array) {
                condition.forEach(function(column) {
                    hash.push(hashCondition(column));
                });
            } else if (condition === null || condition === undefined) {
                hash.push(condition);
            } else {
                condition.forEach(function(key, val) {
                    hash.push(key + ':' + hashCondition(val));
                });
            }
            break;
        case 'string':
...
module.exports = function(descriptor, model, tableDesc) {
    ...
    var getTable = function(schema, table, condition) {
        var hash = '"' + schema + '"."' + table + '":' + hashCondition(condition);
        if (tables[hash] !== undefined) {
            return tables[hash];
        } else {
            var alias = table;
    ...
    var join = function(parent, key, direction, condition) {
        ...
        if (condition === undefined) {
            // we need to generate the alias before generating the condition
            // for the alias we need a condition, so we make up something
            // arbitrary, but unique here. Note that this won't match up with
            // manually created equal conditions...
