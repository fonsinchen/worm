...
exports.rename = function(column) {
  var verbatim = exports.verbatim();
  return {
    bind : function(model, key) {
      var bound = verbatim.bind(model, column);
      var verbatim_extract = bound.extract;
      bound.extract = function(row, results) {
        results[key] = verbatim_extract(row, {})[column];
      }
      return bound;
    }
  }
...
exports.transform = function(extract, insert) {
  ...
  return {
    bind : function(model, key) {
      var bound = verbatim.bind(model, key);
      var verbatim_extract = bound.extract;
      bound.extract = function(row, results) {
        return extract(verbatim_extract(row, results));
      }
      //TODO: value list, column list, name=val
      return bound;
    }
  }
};
...
exports.bloat = function(descriptor) {
  ...
  return {
    bind : function(model, key, info) {
      return require('bind').sub(descriptor, model, {
        ...
        bind : function(model, keyName, info) {
            ...
            return {
                aliasedTables : function(tables, info) {
                    ...
                    var subInfo = child.aliasedTables(tables, info);
                    var fkey = (bindKey === 'lkeys' ? subInfo.table().fkeys[key.name] : key);
                ...
                aliasedColumns : child.aliasedColumns,
...
exports.many = function(descriptor, condition) {
    return join(descriptor, condition, 'lkeys');
};

exports.enumerate = function(column, condition) {
    var many = exports.many({column:1}, condition);
    return {
        bind : function(model, keyName, info) {
            var bound = many.bind(model, keyName, info);
            var bound_extract = bound.extract;
            bound.extract = function(row, results) {
                var sub = bound_extract(row, {});
                if (results[keyName] === undefined) {
                    results[keyName] = [sub[keyName]];
                } else {
                    results[keyName].push(sub[keyName]);
                }
            };
        }
    };
...
exports.one = function(descriptor, condition) {
    return join(descriptor, condition, 'fkeys');
};

exports.flatten = function(descriptor, condition) {
    var one = exports.one(descriptor, condition);
    return {
        bind : function(model, keyName, info) {
            var bound = one.bind(model, keyName, info);
            var bound_extract = bound.extract;
            bound.extract = function(row, results) {
                return bound_extract(row, {
                    keyName : results
                });
            };
            return bound;
