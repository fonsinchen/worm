...
exports = function(driver, bound, where, limit, offset) {
    ...
    return {
        ...
        "select" : function(params, rowCallback, endCallback) {
            var result = {};
            driver.run(getSelect(), params, function(row) {
                bound.extract(row, result);
                // if no explicit ordering given order by primary key of master
                // table, then check for each row if pkey has changed. If so
                // call callback with previous object
