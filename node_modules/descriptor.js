...
module.exports = function(qTableName, structure, descriptor) {
    ...
    if (typeof(qTableName) === 'string') qTableName = qTableName.split('.');
    if (qTableName.length === 1) {
        schema = structure[0];
        if (schema === undefined) {
            schema = structure;
        ...
        tableName = qTableName[0];
    } else if (qTableName.length === 2) {
        schemaName = qTableName[0];
        schema = structure[schemaName];
        tableName = qTableName[1];
    }
